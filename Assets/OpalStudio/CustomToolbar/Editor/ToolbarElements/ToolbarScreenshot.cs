      ...
      sealed internal class ToolbarScreenshot : BaseToolbarElement
      {
            ...
            public override void OnDrawInToolbar()
            {
                  if (EditorGUILayout.DropdownButton(_buttonContent, FocusType.Passive, ToolbarStyles.CommandButtonStyle, GUILayout.Width(this.Width)))
                  {
                        ...
                        // --- Game View Options ---
                        menu.AddItem(new GUIContent("Capture Game View/Current Resolution"), false, static () => CaptureGameView(1));
                        menu.AddSeparator("Capture Game View/");
                        menu.AddItem(new GUIContent("Capture Game View/HD (1920x1080)"), false, static () => CaptureGameViewAtResolution(1920, 1080));
                        menu.AddItem(new GUIContent("Capture Game View/4K (3840x2160)"), false, static () => CaptureGameViewAtResolution(3840, 2160));
                        ...
                        {
                              menu.AddItem(new GUIContent("Capture Scene View/Opaque Background"), false, static () => CaptureSceneView(false));
                              menu.AddItem(new GUIContent("Capture Scene View/Transparent Background"), false, static () => CaptureSceneView(true));
                        }
                        else
                        {
                              menu.AddDisabledItem(new GUIContent("Capture Scene View/Opaque Background (No Scene View active)"));
                              menu.AddDisabledItem(new GUIContent("Capture Scene View/Transparent Background (No Scene View active)"));
                        }
            ...
            private static void CaptureGameViewAtResolution(int width, int height)
            {
                  ...
                  if (mainCamera == null)
                  {
                        Debug.LogWarning("[CustomToolbar] Cannot capture Game View at resolution: no main camera found in scene.");
